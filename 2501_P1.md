...
由于原试卷内容较长，我将分题给出参考答案。部分题目（如开放题）需要自主发挥，我会提供关键思路和示例答案。
...
Prob <- 2 * pnorm(z) - 1  # Pr(|Z| < z)
snd <- tibble(z = z, Prob = round(Prob, 6))
```

**(b) 构造 80% 置信区间**  
已知 σ = 5，n = 9，数据：
```
11.1, 13.2, 9.8, 12.1, 10.9, 12.2, 14.3, 10.8, 11.7
```
样本均值：
```
mean = (11.1 + 13.2 + 9.8 + 12.1 + 10.9 + 12.2 + 14.3 + 10.8 + 11.7) / 9 ≈ 11.79
```
80% 置信区间对应 z* 值（查表 `snd`，Pr(|Z|<z) = 0.8 时 z ≈ 1.28）：
```
CI = mean ± z* × σ / √n
   = 11.79 ± 1.28 × 5 / √9
   = 11.79 ± 2.133
...
#### **(2) 打印所有大于2且小于n的质数**
```r
print((2:n)[sapply(2:n, function(x) all(x %% 2:sqrt(x) != 0))])
```
或更简洁（使用 `isprime` 需安装 `gmp`，此处用基础 R）：
```r
print((2:n)[sapply(2:n, function(x) sum(x %% 2:(x-1) == 0) == 0)])
```
...
**(a) 硬币公平性检验**  
n = 100，H = 61，T = 39，p = 0.5  
检验统计量：
```
z = (61 - 50) / sqrt(100 * 0.5 * 0.5) = 2.2
```
查 `snd` 表，z = 2.2 对应 Pr(|Z|<2.2) ≈ 0.972，所以 p-value ≈ 2 × (1 - 0.972) / 2?  
更准确：  
p-value = 2 × P(Z > 2.2) ≈ 2 × 0.0139 = 0.0278 < 0.05  
**结论：拒绝原假设，硬币不公平。**
...
关系：y = -x（完全线性相关），但协方差：
```
cov = mean((x - mean(x)) * (y - mean(y))) = mean(x * (-x)) = -var(x) ≠ 0? 
```
需调整：  
取 x = [-1, 0, 1]，y = [1, 0, -1]  
cov = ((-1)(1) + (0)(0) + (1)(-1))/3 = -2/3 ≠ 0  
实际上协方差为零需满足 E[(x-μx)(y-μy)]=0，可构造对称分布，如：
```
x = [-2, -1, 1, 2]
y = [1, 2, -2, -1]
```
此时 μx=0, μy=0，cov = ((-2)(1) + (-1)(2) + (1)(-2) + (2)(-1))/4 = (-2 -2 -2 -2)/4 = -2 ≠ 0  
更简单：x 和 y 正交但非线性关系，如 y = x^2 且 x 对称分布：
```
x = [-1, 0, 1]
y = [1, 0, 1]
```
此时 cov = ((-1)(0) + (0)(0) + (1)(0))/3 = 0，但 y = x^2 完全可预测。  
**答案示例：** x = [-1, 0, 1], y = [1, 0, 1]，cov = 0，关系为 y = x²。
...
**(a) 正则表达式输出**  
a. `"Bookname: A happy teacher"`  
b. `NA`（因为前瞻条件 `?=` 不匹配）  
c. `"UID12345"`  
d. `"eter"`（匹配 "P" 后的字母）  
e. 匹配模式 `{4-7}'\\d{1,2}$` 表示 4-7 个字符后接单引号和1-2位数字结尾，匹配的字符串是 `"5'2"` 和 `"5' 25"`（后者不匹配因为空格），所以 `sum` = 1？  
需检查：`"5'2"` 匹配（长度1？不，{4-7} 要求前面4-7个字符，所以都不匹配），所以 `sum` = 0。
...
```
Accuracy = (TP+TN)/Total = 12/20 = 0.6  
Sensitivity = TP/(TP+FN) = 8/13 ≈ 0.615  
Specificity = TN/(TN+FP) = 4/7 ≈ 0.571  
F1 = 2TP/(2TP+FP+FN) = 16/(16+3+5) = 16/24 ≈ 0.667
```

**(iii) 能否保证 100% 特异度？**  
能，只需将所有样本预测为 N，则 FP=0，特异度=1。

**(iv) 能否保证 100% 灵敏度？**  
能，只需将所有样本预测为 Y，则 FN=0，灵敏度=1。

**(v) 证明 100% 灵敏度和特异度 ⇒ 100% 准确度**  
若 Sensitivity=1，则 FN=0；若 Specificity=1，则 FP=0。  
因此所有预测正确，Accuracy=1。
...
library(tidyr)
marksdf <- marksdf %>%
  separate(Marks, into = c("Assessment", "Score"), sep = "_") %>%
  pivot_wider(names_from = Assessment, values_from = Score)
```

**(c) 检测无效条目**  
```r
invalid_uid <- !str_detect(marksdf$UID, "^U\\d{6}$")
invalid_subclass <- !marksdf$Subclass %in% c("A", "B", "C", "D")
```

**(d) 修正 UID**  
```r
marksdf$UID <- str_replace(marksdf$UID, "^[A-TV-Z]", "U")
```

**(e) 计算加权分数**  
```r
marksdf$`Weighted Mark` <- 0.15 * as.numeric(marksdf$A1) +
                           0.15 * as.numeric(marksdf$A2) +
                           0.20 * as.numeric(marksdf$MidTerm) +
                           0.50 * as.numeric(marksdf$Final)
```
...
**(b) 低发区也在 rural counties 的可能原因**  
- 人口基数小：随机波动大，小样本易出现极端值  
- 健康移民效应：健康人群留在乡村，病患迁往城市  
- 数据统计误差：小人口分母小，发病率计算不稳定  
- 环境差异：某些 rural 地区环境更清洁  

**(c) 图表提示**  
散点图显示：人口越小的县，癌症率的变异越大（极端值多）。这与 (b) 一致：小样本统计噪声大，易出现极高或极低值。

---

如果需要我提供完整代码、图表或进一步解释，请告诉我。