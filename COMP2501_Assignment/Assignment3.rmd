...
![](images/clipboard-1179531002.png)
...
Yes, probability will approach 1 as n approached infinity. y-axes intersection is 0.9504273 is very close to 1.

```{r, cache=TRUE}
step_intervals <- c(1e3,2e3,5e3,1e4,2e4,5e4,1e5)
ecdf_values <- sapply(step_intervals, function(n) {
  mean(return_times <= n, na.rm = TRUE)
})
analysis_df <- data.frame(
  steps = step_intervals,
  prob_return = ecdf_values,
  inv_log_n = 1 / log(step_intervals)
)
#regression
model <- lm(prob_return ~ inv_log_n, data = analysis_df)
summary_model <- summary(model)

analysis_df |> ggplot(aes(x = inv_log_n, y = prob_return)) +
  geom_point(size = 3) +
  geom_smooth(method = "lm", se = TRUE, color = "blue", alpha = 0.2) +
  labs(
    title = "Probability of Return vs 1/log(n)",
    x = "1 / log(n)",
    y = "Prob(back to origin)"
  )
...
No, eCDF level up around 0.2\~0.3 as number of steps goes to infinity.
