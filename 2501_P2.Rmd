...
library(lubridate)
library(tidytext)
library(textdata) #unnest_tokens()

all_tweets <- jsonlite::fromJSON("trump_tweets.json",
                                 simplifyDataFrame = TRUE)
trump_tweets <- all_tweets |>
  filter(isRetweet == "f" & !str_detect(text, '^["\']')) |>
  mutate(date = ymd_hms(date))
#in target devices, time interval
...
# 词频统计
top_words <- all_campaign_tweet_words_no_number |>
  count(word) |>
  top_n(10, n) |>
  arrange(desc(n))
```
...
using one variable to indicate another one
...
## 16.3.Association vs Causation

# Lec17-Machine learning

## 17.1.Basics

## 17.2.Evaluations

## 17.3.Decision boundary
