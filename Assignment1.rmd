...
```{r}
murders |> 
  mutate(population_group = ifelse(population <= median(population), "Above median population", "Below median population")) |> 
...
murders |> 
  ...
  geom_point() + 
  geom_text_repel(aes(label = state), max.overlaps = Inf) + 
  scale_x_log10() +
  scale_y_log10() +
